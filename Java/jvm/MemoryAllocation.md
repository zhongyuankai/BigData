...
在 `Netty` 中所有的 IO（nio） 操作都会通过 `Native` 函数直接分配堆外内存。
...
- XX参数：
	- Boolean类型：公式：`-XX：+或者-  某个属性值`，如是否打印GC收集细节: `-XX：+PrintGCDetails`  `-XX：-PrintGCDetails`；
	- KV设值类型：公式：`-XX：属性key=属性值value`，如设置元空间大小：`-XX:MetaspaceSize=128m`；

`jinfo` 查看当前运行程序的配置，公式：`jinfo -flag 配置项 进程编号`；如查看是否使用串行垃圾回收器：`jinfo -flag UseSerialGC 进程号`；`jinfo -flags 进程编号` 查看多个配置；
...
查看JVM默认值：`-XX:+PrintCommandLineFlags`
...
- `-Xss=256k` 线程栈大小。
- `-XX:SurvivoRatio=8` 设置eden和s0/s1空间比例，eden:s0:s1=8:1:1
